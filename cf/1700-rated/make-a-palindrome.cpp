...
void solve() {
    ...
    int kthSmall = sorted[k - 1];
    // debug(kthSmall);

    vector<int> filtered;
    for(auto val: a) {
        if(val > kthSmall) continue;
    ...
    while(left <= right) {
        int leftValue = filtered[left];
        int rightValue = filtered[right];
        ...
        if(leftValue == kthSmall) left ++;
        else if(rightValue == kthSmall) right --;
        else {
            possible = false; 
            break;
        }
    ...
    int remainingSize = filtered.size() - erased;
    if(remainingSize < k - 1) possible = false;
