...
long long calc(int node, int par) {
    debug(node, par);

    int oneNodeChilds = 0;
    int multipleNodeChilds = 0;
    ...
    for(int child: g[node]) {
        ...
        multipleNodeChilds ++;
        answer = mul(answer, calc(child, node));
    }

    if(multipleNodeChilds > 2) answer = 0;
    else if(multipleNodeChilds) {
        if(oneNodeChilds) answer = mul(answer, 2);
    }

    answer = mul(answer, FACT[oneNodeChilds]);
...
void solve() {
    ...
    long long answer = mul(2, calc(1, -1));
