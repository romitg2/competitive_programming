...
void solve() {
    ...
    for(int i = n; i >= 0; i --) {
        ...
        dp[i] = dp[i + 1];
        int currIdx = (lower_bound(all(same[v[i]]), i) - same[v[i]].begin());
        int maxIdx = same[v[i]].size();
        int nextIdx = currIdx + v[i] - 1;
        int next = same[v[i]][nextIdx] + 1;
        
        int maxsame = maxIdx - currIdx;
        if(maxsame >= v[i]) dp[i] = max(dp[i], v[i] + dp[next]);
    }
