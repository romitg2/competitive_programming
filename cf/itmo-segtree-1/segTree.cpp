...
{
    FACT[0] = 1;
    for (int i = 1; i < NCRN; i++)
        FACT[i] = mul(i, FACT[i - 1]);
    IFACT[NCRN - 1] = binExp(FACT[NCRN - 1], mod - 2);
    for (int i = NCRN - 2; i >= 0; i--)
...
class SegTree
{
    ...
    void query(int node, int left, int right, int l, int r)
    {
        ...
        {
            int mid = (left + right) >> 1;
            int leftSeg = query(2 * node, left, mid, l, r);
            int rightSeg = query(2 * node + 1, mid + 1, right, l, r);

            int answer = leftSeg + rightSeg;
