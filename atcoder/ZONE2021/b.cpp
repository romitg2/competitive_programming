...
bool check(double mid, double d, double h, vector<pair<int, int>>& obstacles) {
    if(mid >= h) return true;

    double thetaRequired = (h - mid) / d;

    for(pair<int, int>& obs: obstacles) {
        double theta = ((double)obs.second - mid) / (double)obs.first;
        if(theta > thetaRequired) return false;
    }
...
void solve() {
    ...
    for(int i = 0; i < 1000; i ++) {
        ...
        if(check(mid, d, h, obstacles)) top = mid;
        else bottom = mid;
    }
    
    cout << fixed << setprecision(10) << top << endl;
