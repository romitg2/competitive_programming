...
bool check(vector<vector<int>>& candidates, int mid) {
    ...
    for(int i = n; i >= 0; i --) {
        for(int j = 0; j < (1 << 5); j ++) {
            for(int k = 0; k < 4; k ++) {
                if(i == n) {
                    if(j != (1 << 5) - 1) dp[i][j][k] = false;
                    else dp[i][j][k] = true;
                    continue;
                }

                int mask = 0;
                for(int idx = 0; idx < candidates[i].size(); idx ++) {
                    if(candidates[i][idx] >= mid) mask |= (1 << idx);
                }

                dp[i][j][k] |= dp[i + 1][j][k];
                if(k) dp[i][j][k] |= dp[i + 1][(j | mask)][k - 1];
            }
        }
    }
...
void solve() {
    ...
    vector<vector<int>> candidates(n, vector<int> (5));

    for(int i = 0; i < n; i ++) {
        for(int j = 0; j < 5; j ++) cin >> candidates[i][j];
    }
